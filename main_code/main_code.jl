...
sub_solver_Gurobi = optimizer_with_attributes(Gurobi.Optimizer, "OutputFlag" => false, "Presolve" => 0)
sub_solver_HiGHS = optimizer_with_attributes(HiGHS.Optimizer, "output_flag" => false)
solver_Gurobi = Gurobi.Optimizer
solver_HiGHS = HiGHS.Optimizer
...
scens_list = [50, 200, 500, 1000, 2000]
time_list = [50]
sub_solver_list = [sub_solver_Gurobi, sub_solver_HiGHS]
solver_names = ["Gurobi", "HiGHS"]

# Loop over time values
for t in time_list
    println("=== Starting time = $t ===")
...
# t_HiGHS_fast, ba_HiGHS_fast = run_MP(15, sub_solver_Gurobi; relax = false, scens = 500, multicut = true)
...
# t_Gurobi_BD, ba_Gurobi_BD = run_Plasmo(15, sub_solver_Gurobi; relax = false, scens = 500, multicut = true)
