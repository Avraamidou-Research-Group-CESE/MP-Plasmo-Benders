...
function BendersAlgorithm(
    ...
    time_init = @elapsed begin
        ...
        optimizer.ext["timer_master"] = [0.]
        optimizer.ext["timer_optimize"] = [0.]
...
function _forward_pass!(optimizer::BendersAlgorithm)
    ...
    t_root = @elapsed JuMP.optimize!(root_object)

    optimizer.ext["timer_master"][1] += t_root
    optimizer.ext["timer_optimize"][1] += t_root
